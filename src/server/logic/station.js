...
module.exports = function(router) {
  ...
  router.delete('/station/user/delete', (req, res) =>
    res.json({
      code: 0,
      errorMsg: 'string',
      result: 0,
  ...
  router.get('/station/alarm', (req, res) => {
    ...
    const result = _.map(_.range(0, count), (v, i) =>
      Mock.mock({
        ...
        'userName|1': ['张三', '王五', '赵六', '李四'],
        'deptName|1': ['心血管科', '脑科', '皮肤科', '外科'],
        'status|1': ['0', '1'],
        gmtCreatetime: Random.date('yyyy-MM-dd hh:mm:ss')
      })
    );
  ...
  router.get('/station/notice', (req, res) => {
    ...
    const result = _.map(_.range(0, count), (v, i) =>
      Mock.mock({
        id: i,
        'title|1': ['遇“医闹”，摇一摇，保安立刻到', '找同事更便捷', '随时随地，重要公告一个都不漏'],
        'userName|1': ['张三', '王五', '赵六', '李四'],
        'author|1': ['心血管科', '脑科', '皮肤科', '外科'],
        'isTop|1': [0, 1],
        gmtCreatetime: Random.date('yyyy-MM-dd hh:mm:ss')
      })
    );
